...
abstract class BaseReadAloudService : BaseService(),
    ...
    companion object {
        ...
        @JvmStatic
        var currentSpeakCharPos: Int = 0

        @JvmStatic
        var currentParagraphText: String = ""

        /**
         * 当前段落之前的段落列表（按顺序，越靠后越接近当前段落）
         */
        @JvmStatic
        var previousParagraphs: List<String> = emptyList()

        /**
         * 当前段落之后的段落列表（按顺序，越靠前越接近当前段落）
         */
        @JvmStatic
        var nextParagraphs: List<String> = emptyList()

        @JvmStatic
        var readAloudNumber: Int = 0

        /**
         * 句子分隔标点：朗读到这些标点后会有明显停顿，适合作为句子边界
         */
        @JvmStatic
        val sentenceEndMarks = charArrayOf(
            '\n', '。', '！', '？', '.', '!', '?', '…', '」', '』', '」', '》'
        )

        /**
         * 句内停顿标点：朗读到这些标点后会有短暂停顿，也可作为字幕切分点
         */
        @JvmStatic
        val clauseEndMarks = charArrayOf(
            '，', '、', '；', '：', ',', ';', ':', '—'
        )
         ...
         */
        @JvmStatic
        fun isSentenceEnd(c: Char): Boolean {
            return c in sentenceEndMarks
        }
         ...
         */
        @JvmStatic
        fun isClauseEnd(c: Char): Boolean {
            return c in clauseEndMarks
        }

        /**
         * 从文本中提取当前在读的子句（按标点符号分割）
         * 优先按句号等大标点分割，其次按逗号等小标点分割
         * @param text 段落完整文本
         * @param position 当前朗读字符在段落中的位置
         * @return 当前正在朗读的子句文本
         */
        @JvmStatic
        fun extractCurrentSentence(text: String, position: Int): String {
            if (text.isEmpty() || position < 0) return ""
            val len = text.length
            val pos = position.coerceIn(0, len - 1)

            // 向前查找子句起始位置
            var start = pos
            while (start > 0) {
                val c = text[start - 1]
                if (isSentenceEnd(c) || isClauseEnd(c)) {
                    break
                }
                start--
            }
            ...
            return text.substring(start, end.coerceAtMost(len)).trim()
        }
    ...
    private fun newReadAloud(play: Boolean, pageIndex: Int, startPos: Int) {
        execute(executeContext = IO) {
            ...
            val paragraphText =
                if (nowSpeak in contentList.indices) contentList[nowSpeak] else ""
            ...
            currentSpeakCharPos = pos
            currentParagraphText = paragraphText
            currentSpeakingText = extractCurrentSentence(paragraphText, currentSpeakCharPos)
            // 填充前后段落列表（歌词效果用）
            val prevCount = 5
            val nextCount = 5
            previousParagraphs = if (nowSpeak > 0) {
                val start = maxOf(0, nowSpeak - prevCount)
                contentList.subList(start, nowSpeak)
            } else emptyList()
            nextParagraphs = if (nowSpeak < contentList.lastIndex) {
                val end = minOf(contentList.size, nowSpeak + nextCount + 1)
                contentList.subList(nowSpeak + 1, end)
            } else emptyList()
            launch(Main) {
                upMediaMetadata()
    ...
    fun upTtsProgress(progress: Int) {
        currentParagraphText = if (nowSpeak in contentList.indices) {
            contentList[nowSpeak]
        } else ""
        currentSpeakingText = if (nowSpeak in contentList.indices) {
            val paragraph = contentList[nowSpeak]
            extractCurrentSentence(paragraph, currentSpeakCharPos)
        } else ""
        // 填充前后段落列表（歌词效果用）
        val prevCount = 5
        val nextCount = 5
