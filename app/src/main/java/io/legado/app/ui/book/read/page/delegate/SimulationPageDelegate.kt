...
class SimulationPageDelegate(readView: ReadView) : HorizontalPageDelegate(readView) {
    ...
    // 阴影颜色组
    private val mFolderShadowColors = intArrayOf(0x333333, -0x4fcccccd)
    private val mBackShadowColors = intArrayOf(-0xeeeeef, 0x111111)
    private val mFrontShadowColors = intArrayOf(-0x7feeeeef, 0x111111)

    // 渐变阴影画笔（改用 Paint + LinearGradient 直接绘制，避免 GradientDrawable 在 OPPO/API28 上的数组不同步崩溃）
    ...
    /**
     * 使用 Paint + LinearGradient 直接绘制渐变阴影，
     * 避免 GradientDrawable 在 OPPO/API28 上的内部数组不同步导致崩溃。
     */
    private fun drawShadowGradient(
        canvas: Canvas,
        colors: IntArray,
        ...
        when (orientation) {
            GradientDrawable.Orientation.LEFT_RIGHT -> {
                x0 = left.toFloat(); y0 = top.toFloat()
                x1 = right.toFloat(); y1 = top.toFloat()
            }
            GradientDrawable.Orientation.RIGHT_LEFT -> {
                x0 = right.toFloat(); y0 = top.toFloat()
                x1 = left.toFloat(); y1 = top.toFloat()
            }
            GradientDrawable.Orientation.TOP_BOTTOM -> {
                x0 = left.toFloat(); y0 = top.toFloat()
                x1 = left.toFloat(); y1 = bottom.toFloat()
            }
            GradientDrawable.Orientation.BOTTOM_TOP -> {
                x0 = left.toFloat(); y0 = bottom.toFloat()
                x1 = left.toFloat(); y1 = top.toFloat()
            }
            else -> {
                x0 = left.toFloat(); y0 = top.toFloat()
        ...
        canvas.drawRect(left.toFloat(), top.toFloat(), right.toFloat(), bottom.toFloat(), mShadowPaint)
    ...
    private fun drawCurrentBackArea(
        ...
        mPath1.close()
        val folderOrientation: GradientDrawable.Orientation
        val left: Int
        val right: Int
        if (mIsRtOrLb) {
            left = (mBezierStart1.x - 1).toInt()
            right = (mBezierStart1.x + f3 + 1).toInt()
            folderOrientation = GradientDrawable.Orientation.LEFT_RIGHT
        } else {
            left = (mBezierStart1.x - f3 - 1).toInt()
            right = (mBezierStart1.x + 1).toInt()
            folderOrientation = GradientDrawable.Orientation.RIGHT_LEFT
        }
        canvas.save()
        ...
        drawShadowGradient(
            canvas, mFolderShadowColors, folderOrientation,
    ...
    private fun drawCurrentPageShadow(canvas: Canvas) {
        ...
        var leftX: Int
        var rightX: Int
        var frontOrientation: GradientDrawable.Orientation
        if (mIsRtOrLb) {
            leftX = mBezierControl1.x.toInt()
            rightX = (mBezierControl1.x + 25).toInt()
            frontOrientation = GradientDrawable.Orientation.LEFT_RIGHT
        } else {
            leftX = (mBezierControl1.x - 25).toInt()
            rightX = (mBezierControl1.x + 1).toInt()
            frontOrientation = GradientDrawable.Orientation.RIGHT_LEFT
        }
        ...
        drawShadowGradient(
            canvas, mFrontShadowColors, frontOrientation,
        ...
        if (mIsRtOrLb) {
            ...
            frontOrientation = GradientDrawable.Orientation.TOP_BOTTOM
        } else {
            leftX = (mBezierControl2.y - 25).toInt()
            rightX = (mBezierControl2.y + 1).toInt()
            frontOrientation = GradientDrawable.Orientation.BOTTOM_TOP
        }
    ...
    private fun drawNextPageAreaAndShadow(
        ...
        val leftX: Int
        val rightX: Int
        val backOrientation: GradientDrawable.Orientation
        if (mIsRtOrLb) { //左下及右上
            leftX = mBezierStart1.x.toInt()
            rightX = (mBezierStart1.x + mTouchToCornerDis / 4).toInt()
            backOrientation = GradientDrawable.Orientation.LEFT_RIGHT
        } else {
            leftX = (mBezierStart1.x - mTouchToCornerDis / 4).toInt()
            rightX = mBezierStart1.x.toInt()
            backOrientation = GradientDrawable.Orientation.RIGHT_LEFT
        }
        canvas.save()
        canvas.clipPath(mPath0)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        ...
        canvas.rotate(mDegrees, mBezierStart1.x, mBezierStart1.y)
        drawShadowGradient(
            canvas, mBackShadowColors, backOrientation,
