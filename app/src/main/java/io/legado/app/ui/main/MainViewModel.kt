...
class MainViewModel(
    ...
    fun onPrefClickEvent(event: PrefClickEvent) {
        when (event) {
            ...
            PrefClickEvent.OpenAiNovelFilter -> _effects.tryEmit(MainEffect.NavigateToAiNovelFilter)
...
sealed interface MainEffect {
    ...
    data object NavigateToAiNovelFilter : MainEffect
}
...
private fun MainViewModel.readMainUiState(): MainUiState {
    ...
    val showReadRecord = context.getPrefBoolean(PreferKey.showReadRecord, true)
    val destinations = MainDestination.mainDestinations.filter {
        when (it) {
            ...
            MainDestination.ReadRecord -> showReadRecord
            else -> true
        }
    }.toImmutableList()
    return MainUiState(
