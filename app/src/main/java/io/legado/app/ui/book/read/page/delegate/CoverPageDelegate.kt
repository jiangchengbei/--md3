...
class CoverPageDelegate(readView: ReadView) : HorizontalPageDelegate(readView) {
    private val shadowColors = intArrayOf(0x66111111, 0x00000000)
    private val shadowPaint = Paint()

    private fun updateShadowBounds(left: Int, top: Int, right: Int, bottom: Int) {
    ...
    private var shadowTop = 0
    private var shadowRight = 0
    private var shadowBottom = 0
    ...
    private fun addShadow(left: Float, canvas: Canvas) {
        ...
        canvas.withTranslation(dx) {
            canvas.drawRect(
                shadowLeft.toFloat(), shadowTop.toFloat(),
                shadowRight.toFloat(), shadowBottom.toFloat(),
                shadowPaint
            )
        }
    }
    ...
    override fun setViewSize(width: Int, height: Int) {
        ...
        shadowTop = 0
        shadowRight = 30
        shadowBottom = viewHeight
        updateShadowBounds(0, 0, 30, viewHeight)
    }
