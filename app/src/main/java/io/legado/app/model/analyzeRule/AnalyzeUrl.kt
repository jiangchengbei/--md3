...
class AnalyzeUrl(
    ...
    /** 低于此大小的响应始终允许加载，避免内存极低时连小响应也拒绝 */
    private val MIN_CONTENT_LENGTH_CHECK = 5 * 1024 * 1024L // 5MB
    ...
    suspend fun getByteArrayAwait(): ByteArray {
        ...
        return try {
            ...
            // 检查可用堆内存：如果响应体超过 5MB 且超过可用内存的 70%，拒绝加载
            if (contentLength > MIN_CONTENT_LENGTH_CHECK) {
                ...
                if (contentLength > availableMemory * 0.7) {
                    response.close()
                    throw NoStackTraceException(
                        "响应体过大(${contentLength} bytes)，可用内存不足(${availableMemory} bytes)，无法加载"
                    )
                }
            }
            body.bytes()
        } catch (e: OutOfMemoryError) {
            // 即使通过了 contentLength 检查（或 contentLength 未知），
            // 实际分配时仍可能 OOM，给出明确错误信息
            throw NoStackTraceException("内存不足，无法下载数据：${e.message}")
        }
    }
