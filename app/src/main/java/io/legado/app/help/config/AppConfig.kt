...
//        get() = appCtx.getPrefBoolean(PreferKey.immNavigationBar, true)
    ...
    /** 语速选项：从0.25倍到3倍，每级0.25递增 */
    val speechRateValues = floatArrayOf(0.25f, 0.5f, 0.75f, 1.0f, 1.25f, 1.5f, 1.75f, 2.0f, 2.25f, 2.5f, 2.75f, 3.0f)

    /** 默认语速索引（1.0x = 索引3） */
    const val defaultSpeechRate = 3

    /** 当前语速索引（0-11，对应 speechRateValues） */
    var ttsSpeechRate: Int
        get() = appCtx.getPrefInt(PreferKey.ttsSpeechRate, defaultSpeechRate)
        set(value) {
            appCtx.putPrefInt(PreferKey.ttsSpeechRate, value.coerceIn(0, speechRateValues.size - 1))
        }

    /** 当前实际语速浮点值 */
    val speechRateFloat: Float get() = speechRateValues.getOrElse(ttsSpeechRate) { 1.0f }

    var readAloudShowCover: Boolean
        get() = appCtx.getPrefBoolean(PreferKey.readAloudShowCover, true)
        set(value) {
            appCtx.putPrefBoolean(PreferKey.readAloudShowCover, value)
        }

    var readAloudCoverSize: Int
        get() = appCtx.getPrefInt(PreferKey.readAloudCoverSize, 300)
        set(value) {
            appCtx.putPrefInt(PreferKey.readAloudCoverSize, value)
        }

    var readAloudSubtitleFontSize: Int
        get() = appCtx.getPrefInt(PreferKey.readAloudSubtitleFontSize, 0)
        set(value) {
            appCtx.putPrefInt(PreferKey.readAloudSubtitleFontSize, value)
        }

    var readAloudCoverCarousel: Boolean
        get() = appCtx.getPrefBoolean(PreferKey.readAloudCoverCarousel, true)
        set(value) {
            appCtx.putPrefBoolean(PreferKey.readAloudCoverCarousel, value)
        }
    ...
    val speechRatePlay: Float get() = if (ttsFlowSys) speechRateValues[defaultSpeechRate] else speechRateFloat
