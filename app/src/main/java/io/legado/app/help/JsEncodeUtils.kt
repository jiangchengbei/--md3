...
//******************消息摘要/散列消息鉴别码************************//
     ...
     *
     * @param data 被摘要数据
     * @param algorithm 签名算法（如 MD5, SHA-1, SHA-256 等）
     * @return 16进制字符串
     */
    fun digestHex(
        data: String,
        ...
        return if (data.length <= LARGE_STRING_THRESHOLD) {
            DigestUtil.digester(algorithm).digestHex(data)
        } else {
            chunkedDigestHex(data, algorithm)
        }
    }
    ...
    fun digestBase64Str(
        ...
        return if (data.length <= LARGE_STRING_THRESHOLD) {
            ...
            Base64.encodeToString(chunkedDigest(data, algorithm), Base64.NO_WRAP)
        }
    }
     ...
     *
     * @param data 被摘要数据
     * @param algorithm 签名算法（如 HmacMD5, HmacSHA256 等）
     * @param key 密钥
     * @return 16进制字符串
     */
    ...
    fun HMacHex(
        ...
        return if (data.length <= LARGE_STRING_THRESHOLD) {
            HMac(algorithm, key.toByteArray()).digestHex(data)
        } else {
            chunkedHMacHex(data, algorithm, key)
        }
    }
    ...
    fun HMacBase64(
        ...
        return if (data.length <= LARGE_STRING_THRESHOLD) {
            ...
            Base64.encodeToString(chunkedHMac(data, algorithm, key), Base64.NO_WRAP)
        }
    }

    // ===== 分块处理私有方法 =====

    private fun chunkedDigestHex(str: String, algorithm: String): String {
        val md = MessageDigest.getInstance(algorithm)
        updateChunked(md, str)
        return HexUtil.encodeHexStr(md.digest())
    }

    private fun chunkedDigest(str: String, algorithm: String): ByteArray {
        val md = MessageDigest.getInstance(algorithm)
        updateChunked(md, str)
        return md.digest()
    }

    private fun chunkedHMacHex(str: String, algorithm: String, key: String): String {
        val mac = Mac.getInstance(algorithm)
        mac.init(SecretKeySpec(key.toByteArray(), algorithm))
        updateChunkedMac(mac, str)
        return HexUtil.encodeHexStr(mac.doFinal())
    }

    private fun chunkedHMac(str: String, algorithm: String, key: String): ByteArray {
        val mac = Mac.getInstance(algorithm)
        mac.init(SecretKeySpec(key.toByteArray(), algorithm))
        updateChunkedMac(mac, str)
        return mac.doFinal()
    ...
    private fun updateChunked(md: MessageDigest, str: String) {
        var offset = 0
        val len = str.length
        while (offset < len) {
            val end = minOf(offset + CHUNK_SIZE, len)
            md.update(str.substring(offset, end).toByteArray(Charsets.UTF_8))
            offset = end
        }
    }

    private fun updateChunkedMac(mac: Mac, str: String) {
        var offset = 0
        val len = str.length
        while (offset < len) {
            val end = minOf(offset + CHUNK_SIZE, len)
            mac.update(str.substring(offset, end).toByteArray(Charsets.UTF_8))
            offset = end
        }
    }
...
private const val LARGE_STRING_THRESHOLD = 256 * 1024  // 256KB

/** 每次分块处理的字符数 */
private const val CHUNK_SIZE = 4096  // 4KB