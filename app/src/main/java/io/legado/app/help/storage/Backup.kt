    ...
    fun autoBack(context: Context) {
        if (shouldBackup()) {
            Coroutine.async {
                mutex.withLock {
                    if (shouldBackup()) {
                        val backupMode = appCtx.getPrefString(PreferKey.backupSyncMode, "both") ?: "both"
                        val backupZipFileName = getNowZipFileName()
                        val hasBackupOnWebDav = if (backupMode in listOf("both", "webdav")) {
                            kotlin.runCatching {
                        ...
                        if (!hasBackupOnWebDav) {
                            backup(context, AppConfig.backupPath, backupMode)
                        } else {
